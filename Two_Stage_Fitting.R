...
## beta_x, beta_y, beta_z are the coefficients for the splines to the x-axis and y-axis
beta_x = beta_x_new
beta_y = beta_y_new
beta_z = beta_z_new
...
## items used during the EM procedure
x.i.matrix = matrix(x,nrow=length(x),ncol=K,byrow=FALSE)
x.k.matrix = matrix(B %*% beta_x, nrow = N, ncol = length(B %*% beta_x), byrow = TRUE)
y.i.matrix = matrix(y,nrow = length(y), ncol = K, byrow = FALSE)
y.k.matrix = matrix(B %*% beta_y, nrow = N, ncol = length(B %*% beta_y), byrow = TRUE)
z.i.matrix = matrix(z, nrow = length(z), ncol = K, byrow = FALSE)
z.k.matrix = matrix(B %*% beta_z, nrow = N, ncol = length(B %*% beta_z), byrow = TRUE)
